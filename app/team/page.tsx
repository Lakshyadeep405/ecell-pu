...
import { ArrowLeft, Search, Users, Cpu, Palette, Megaphone, Wrench, ShieldCheck, RefreshCw } from "lucide-react";
    ...
    async function loadMembers() {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setMembersList(ALL_MEMBERS); // Fallback to hardcoded core data
        }
      } catch (err) {
        console.error("Error loading squad:", err);
        setMembersList(ALL_MEMBERS); // Fallback
      } finally {
        setLoading(false);
      }
    }
